...
const DatabaseSetupPage: React.FC<DatabaseSetupPageProps> = ({ onRetry, details }) => {
    ...
    return (
        <div className="flex items-center justify-center min-h-screen bg-gray-50 dark:bg-gray-900 p-4">
            <div className="w-full max-w-3xl p-8 space-y-6 bg-white dark:bg-gray-800 rounded-2xl shadow-lg">
                <div className="text-center">
                    <div className="inline-flex items-center justify-center bg-red-100 p-3 rounded-full mb-4">
                       <AlertTriangle className="h-8 w-8 text-red-600" />
                    </div>
                    <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-50">Configuração Inicial Requerida</h1>
                    <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">O aplicativo detectou que o banco de dados não está configurado corretamente. Siga os passos abaixo para começar.</p>
                </div>
                ...
                <div className="space-y-4">
                    ...
                    <Step title="Passo 2: Habilitar a Extensão 'uuid-ossp'" isComplete={isUuidEnabled}>
                        ...
                        <ol className="list-decimal list-inside space-y-1 pl-2">
                            <li>Vá para seu projeto no <a href="https://supabase.com/dashboard" target="_blank" rel="noopener noreferrer" className="text-blue-600 dark:text-blue-400 hover:underline font-semibold">Dashboard do Supabase</a>.</li>
                    ...
                    <div className="border border-gray-200 dark:border-gray-700 rounded-lg">
                        <button onClick={() => setShowDetails(!showDetails)} className="w-full flex justify-between items-center p-3 bg-gray-50 dark:bg-gray-700 hover:bg-gray-200 dark:bg-gray-600 rounded-lg">
                            <h4 className="font-semibold text-gray-900 dark:text-gray-50">Verificar Status Detalhado do Banco</h4>
                            <ChevronRight className={`transform transition-transform ${showDetails ? 'rotate-90' : ''}`} />
                        </button>
                        {showDetails && details && (
                            <div className="p-4 grid grid-cols-1 md:grid-cols-3 gap-4 border-t border-gray-200 dark:border-gray-700">
                                <div>
                                    <h5 className="font-semibold mb-2 flex items-center"><Server size={16} className="mr-2"/> Tabelas</h5>
                                    <div className="space-y-1 max-h-40 overflow-y-auto pr-2">
                                        {details.tables_status?.map((item: any) => <StatusItem key={item.name} {...item} />) ?? <p className="text-xs text-gray-500 dark:text-gray-400">Não foi possível verificar.</p>}
                                    </div>
                                </div>
                                <div>
                                    <h5 className="font-semibold mb-2 flex items-center"><Type size={16} className="mr-2"/> Tipos</h5>
                                    <div className="space-y-1 max-h-40 overflow-y-auto pr-2">
                                        {details.types_status?.map((item: any) => <StatusItem key={item.name} {...item} />) ?? <p className="text-xs text-gray-500 dark:text-gray-400">Não foi possível verificar.</p>}
                                    </div>
                                </div>
                                <div>
                                    <h5 className="font-semibold mb-2 flex items-center"><FunctionSquare size={16} className="mr-2"/> Funções</h5>
                                    <div className="space-y-1 max-h-40 overflow-y-auto pr-2">
                                        {details.functions_status?.map((item: any) => <StatusItem key={item.name} {...item} />) ?? <p className="text-xs text-gray-500 dark:text-gray-400">Não foi possível verificar.</p>}
                                    </div>
                                </div>
                            </div>
                        )}
