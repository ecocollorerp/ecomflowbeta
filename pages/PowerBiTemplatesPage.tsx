...
const AccordionItem: React.FC<{ title: string; children: React.ReactNode }> = ({ title, children }) => {
    ...
    return (
        <div className="border border-gray-200 dark:border-gray-700 rounded-lg">
            <button onClick={() => setIsOpen(!isOpen)} className="w-full flex justify-between items-center p-4 text-left font-semibold text-gray-900 dark:text-gray-50 bg-gray-50 dark:bg-gray-700 hover:bg-gray-200 dark:bg-gray-600 transition-colors">
                <span>{title}</span>
                <ChevronDown className={`transform transition-transform ${isOpen ? 'rotate-180' : ''}`} />
            </button>
            {isOpen && <div className="p-4 bg-white dark:bg-gray-800 prose prose-sm max-w-none text-gray-500 dark:text-gray-400">{children}</div>}
        </div>
    );
};
...
const PowerBiTemplatesPage: React.FC<{ setCurrentPage: (page: string) => void }> = ({ setCurrentPage }) => {
    return (
        <div className="space-y-6">
            <button onClick={() => setCurrentPage('powerbi')} className="flex items-center gap-2 text-sm font-semibold text-blue-600 dark:text-blue-400 hover:underline">
                <ArrowLeft size={18} />
                Voltar para o Dashboard
            </button>
            <div>
                <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-50">Modelos e Instruções para Power BI</h1>
                <p className="text-gray-500 dark:text-gray-400 mt-1">Siga este guia para conectar e criar seus próprios dashboards.</p>
            </div>
            
            <div className="bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
                <h2 className="text-xl font-bold text-gray-900 dark:text-gray-50 mb-4 flex items-center gap-3"><Database size={24}/>Conexão com o Banco de Dados</h2>
                <ol className="list-decimal list-inside space-y-3 text-gray-500 dark:text-gray-400">
                    ...
                    <li>
                        ...
                        <ul className="list-disc list-inside pl-6 mt-2 space-y-1 font-mono text-sm bg-gray-50 dark:bg-gray-700 p-3 rounded-md border">
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
                <h2 className="text-xl font-bold text-gray-900 dark:text-gray-50 mb-4">Modelos de Relatórios (Fórmulas DAX)</h2>
