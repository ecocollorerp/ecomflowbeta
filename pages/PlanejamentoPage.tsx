...
const StepHeader: React.FC<{ number: number, title: string, isComplete: boolean }> = ({ number, title, isComplete }) => (
    <div className="flex items-center">
        <div className={`w-8 h-8 rounded-full flex items-center justify-center mr-3 font-bold ${isComplete ? 'bg-green-600 text-white' : 'bg-blue-600 dark:bg-blue-500 text-white'}`}>
...
const PlanejamentoPage: React.FC<PlanejamentoPageProps> = ({ stockItems, allOrders, skuLinks, produtosCombinados, productionPlans, onSaveProductionPlan, onDeleteProductionPlan, onGenerateShoppingList, currentUser, planningSettings, onSavePlanningSettings, addToast, costCalculations }) => {
    ...
    const handleUpdateProduction = (productCode: string, newRequired: number) => {
        ...
        setProductionPlan(prev => {
            const next = prev.map(item => item.product.code === productCode ? {...item, requiredProduction: value, projectedRevenue: value * (item.avgPrice || 0)} : item);
            
            // Recalculate Scenario Analysis
            const newTotalRevenue = next.reduce((sum, item) => sum + (item.projectedRevenue || 0), 0);
    ...
    const handleUpdatePrice = (productCode: string, newPrice: number) => {
        const value = Math.max(0, newPrice);
        setProductionPlan(prev => {
            const next = prev.map(item => {
                if (item.product.code === productCode) {
                    return {
                        ...item,
                        avgPrice: value,
                        projectedRevenue: item.requiredProduction * value
                    };
                }
                return item;
            });
    ...
    return (
        <div className="space-y-8">
            <div className="flex justify-between items-start flex-wrap gap-4">
                <div>
                    <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-50">Planejamento e Simulação</h1>
                    <p className="text-gray-500 dark:text-gray-400 mt-1">Defina metas financeiras ou de volume e gere planos de produção baseados no histórico.</p>
                </div>
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
                ...
                <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                    <div className="flex items-center gap-4 mb-4">
                        <label className="font-semibold text-gray-900 dark:text-gray-50">Modo:</label>
                        <div className="flex items-center p-1 bg-gray-50 dark:bg-gray-700 rounded-lg">
                            <button onClick={() => setMode('automatico')} className={`px-3 py-1 text-sm rounded-md ${mode === 'automatico' ? 'bg-white dark:bg-gray-800 shadow-sm font-semibold text-blue-600 dark:text-blue-400' : 'text-gray-500 dark:text-gray-400'}`}>Automático (Simples)</button>
                            <button onClick={() => setMode('manual')} className={`px-3 py-1 text-sm rounded-md ${mode === 'manual' ? 'bg-white dark:bg-gray-800 shadow-sm font-semibold text-blue-600 dark:text-blue-400' : 'text-gray-500 dark:text-gray-400'}`}>Manual (Avançado)</button>
                        </div>
                    </div>
                    
                    {mode === 'manual' && (
                         <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 p-4 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-700 rounded-lg">
                            <div className="space-y-4">
                                <h4 className="font-bold text-gray-900 dark:text-gray-50 flex items-center gap-2"><History size={16}/> Base Histórica de Análise</h4>
                ...
                <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                    <div className="flex justify-between items-center mb-4 flex-wrap gap-4">
                        ...
                        <div className="text-sm text-right">
                            {analysisPeriod.start && analysisPeriod.end && <p className="text-gray-500 dark:text-gray-400">Base Histórica: <strong className="text-gray-900 dark:text-gray-50">{new Date(analysisPeriod.start + 'T12:00:00Z').toLocaleDateString()}</strong> até <strong className="text-gray-900 dark:text-gray-50">{new Date(analysisPeriod.end + 'T12:00:00Z').toLocaleDateString()}</strong>.</p>}
                        </div>
                    </div>
                    ...
                    {isCalculating ? <div className="text-center p-8"><Loader2 size={32} className="animate-spin text-blue-600 dark:text-blue-400 mx-auto"/></div> :
                                            ...
                                            <td className="py-3 px-3">
                                                <div className="relative">
                                                    ...
                                                    <input 
                                                        ...
                                                        onChange={e => handleUpdatePrice(item.product.code, parseFloat(e.target.value))} 
                                                        className="w-24 pl-6 p-2 text-right border border-slate-200 rounded-lg text-slate-700 focus:border-indigo-500 outline-none text-sm font-medium" 
                                                    />
                                                </div>
                                            </td>
                                            <td className="py-3 px-3">
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
                 ...
                 <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700 grid grid-cols-1 md:grid-cols-2 gap-8">
                     <div className="space-y-4">
                         <h3 className="font-bold text-gray-900 dark:text-gray-50 flex items-center gap-2"><Package size={18}/> Insumos Necessários</h3>
                         <div className="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-700 max-h-96">
                            <table className="min-w-full bg-white dark:bg-gray-800 text-sm">
                                <thead className="bg-gray-50 dark:bg-gray-700 sticky top-0"><tr>{['Insumo', 'Necessidade', 'Estoque', 'Déficit'].map(h=><th key={h} className="py-2 px-3 text-left font-semibold text-gray-500 dark:text-gray-400">{h}</th>)}</tr></thead>
                                <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                                     {requiredInsumos.length > 0 ? requiredInsumos.map(item => (
                                        <tr key={item.insumo.id} className={item.deficit > 0 ? 'bg-red-50 dark:bg-red-900/30' : ''}>
                                            <td className="py-2 px-3 font-medium text-gray-900 dark:text-gray-50">{item.insumo.name}</td>
                                            <td className="py-2 px-3 text-center text-gray-900 dark:text-gray-50">{item.requiredQty.toFixed(2)}</td>
                                            <td className="py-2 px-3 text-center text-gray-900 dark:text-gray-50">{item.currentStock.toFixed(2)}</td>
                                            <td className={`py-2 px-3 text-center font-bold ${item.deficit > 0 ? 'text-red-700 dark:text-red-300' : 'text-green-700 dark:text-green-300'}`}>{item.deficit.toFixed(2)}</td>
                                        </tr>
                                    )) : <tr><td colSpan={4} className="text-center p-8 text-gray-500 dark:text-gray-400">Ajuste o plano para calcular os insumos.</td></tr>}
                                </tbody>
                            </table>
                         </div>
                     </div>
                     <div className="space-y-4">
                         <h3 className="font-bold text-gray-900 dark:text-gray-50 flex items-center gap-2"><ListTodo size={18}/> Ações e Histórico</h3>
                         <div className="p-4 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-700 rounded-lg space-y-3">
                             <div><label className="text-sm font-medium text-gray-500 dark:text-gray-400">Nome do Plano</label><input type="text" value={planName} onChange={e => setPlanName(e.target.value)} className="w-full mt-1 p-2 border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 rounded-md text-gray-900 dark:text-gray-50" /></div>
                             <div className="flex gap-2">
                         ...
                         <div className="p-4 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-700 rounded-lg">
                             <h4 className="font-medium text-gray-900 dark:text-gray-50 flex items-center gap-2 mb-2"><History size={16}/> Planos Salvos</h4>
                             <div className="space-y-2 max-h-40 overflow-y-auto pr-2">
                                 {productionPlans.map(plan => (
                                     <div key={plan.id} className={`flex justify-between items-center p-2 rounded-md ${activePlan?.id === plan.id ? 'bg-blue-50 dark:bg-blue-900/20 border border-blue-600 dark:border-blue-500' : 'bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700'}`}>
                                         <div>
                                            <p className="font-semibold text-sm text-gray-900 dark:text-gray-50">{plan.name}</p>
                                            <p className="text-xs text-gray-500 dark:text-gray-400">Criado por {plan.createdBy} em {new Date(plan.createdAt).toLocaleDateString()}</p>
                                        </div>
                                         <div className="flex gap-1">
                                            <button onClick={() => handleLoadPlan(plan)} className="p-1 text-blue-700 dark:text-blue-300 hover:bg-gray-200 dark:bg-gray-600 rounded-full"><Send size={14}/></button>
