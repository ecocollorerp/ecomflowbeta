...
const SimpleBarChart: React.FC<{ title: string; data: { label: string; value: number }[] }> = ({ title, data }) => {
    ...
    return (
        <div className="bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm h-full">
            <h3 className="text-lg font-bold text-gray-900 dark:text-gray-50 mb-4">{title}</h3>
            <div className="space-y-4">
                {data.map(({ label, value }) => (
                    <div key={label}>
                        ...
                        <div className="w-full bg-gray-50 dark:bg-gray-700 rounded-full h-4">
                            <div
...
const BiDashboardPage: React.FC<BiDashboardPageProps> = ({ biData, users }) => {
    ...
    const SortableHeader: React.FC<{ label: string, sortKey: keyof BiDataItem }> = ({ label, sortKey }) => {
        ...
        return <th className="py-2 px-3 text-left font-semibold text-gray-500 dark:text-gray-400"><button onClick={() => requestSort(sortKey)} className="flex items-center gap-1">{label}{isSorted && <Icon size={14}/>}</button></th>;
    };
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex flex-wrap gap-4 items-center p-4 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
                ...
                <select value={filters.period} onChange={e => handleFilterChange('period', e.target.value)} className="p-2 text-sm border border-gray-200 dark:border-gray-700 rounded-md bg-white dark:bg-gray-800">
                ...
                <select value={filters.canal} onChange={e => handleFilterChange('canal', e.target.value)} className="p-2 text-sm border border-gray-200 dark:border-gray-700 rounded-md bg-white dark:bg-gray-800"><option value="ALL">Todos Canais</option><option value="ML">ML</option><option value="SHOPEE">Shopee</option><option value="SITE">Site</option></select>
                <select value={filters.operator} onChange={e => handleFilterChange('operator', e.target.value)} className="p-2 text-sm border border-gray-200 dark:border-gray-700 rounded-md bg-white dark:bg-gray-800"><option value="ALL">Todos Operadores</option>{users.map(u => <option key={u.id} value={u.id}>{u.name}</option>)}</select>
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
                <h3 className="text-lg font-bold text-gray-900 dark:text-gray-50 mb-4">Dados Detalhados</h3>
                <div className="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-700 max-h-[500px]">
                    <table className="min-w-full text-sm">
                        ...
                        <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                            {filteredData.map(item => (
                                <tr key={item.id_pedido} className="hover:bg-gray-50 dark:bg-gray-700">
