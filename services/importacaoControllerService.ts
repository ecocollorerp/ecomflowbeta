  ...
  static async buscarPedidosNaoVinculados(
    ...
    try {
      ...
      while (continuarBuscando) {
        let urlBling = `/api/bling/pedidos/vendas?limite=${limite}&pagina=${pagina}`;
        ...
        if (
          pedidosBase.length < limite ||
          todosPedidosBling.length >= quantidadeDesejada * 2 ||
          pagina >= 20
        ) {
      ...
      const avisos: string[] = [];
      const naoVinculadosProcessados: PedidoblingNaoVinculado[] =
        todosPedidosBling
          ...
          .map((pedido: any) => {
            ...
            if (!numeroLojaOriginal) {
              ...
              const match = info.match(
                /(?:Número\s+(?:loja|virtual)|Pedido)\s*[:\-]?\s*([A-Za-z0-9\-_]+)/i
              );
              if (match) numeroLojaOriginal = match[1].trim();
            }

            if (
              canalStr.includes("SHOPEE") ||
              nomeLojaStr.includes("SHOPEE") ||
              numeroLojaOriginal.toUpperCase().includes("SP")
            ) {
              origem = "SHOPEE";
            } else if (
              canalStr.includes("MERCADO") ||
              nomeLojaStr.includes("MERCADO") ||
              numeroLojaOriginal.match(/ML[A-Z0-9]+/i)
            ) {
              origem = "MERCADO_LIVRE";
            } else if (
              canalStr.includes("SITE") ||
              nomeLojaStr.includes("SITE")
            ) {
              origem = "SITE";
            }
      ...
      if (naoVinculadosProcessados.length === 0) {
        ...
        avisos.push(
          `⚠️ Exibindo ${naoVinculadosProcessados.slice(0, quantidadeDesejada).length} de ${naoVinculadosProcessados.length} não vinculados.`
        );
      }

      return {
        pedidosNaoVinculados: naoVinculadosProcessados.slice(
          0,
          quantidadeDesejada
        ),
  ...
  static async buscarPedidosEmAbertoPorPlataforma(
    ...
    try {
      ...
      while (continuarCarregando) {
        ...
        const situacoesParam =
          idsSituacoes && idsSituacoes.length > 0
            ? idsSituacoes.map((id) => `&idsSituacoes[]=${id}`).join("")
            : "&idsSituacoes[]=6&idsSituacoes[]=15"; // default aberto + andamento

        let url = `/api/bling/pedidos/vendas?limite=${limite}&pagina=${pagina}${situacoesParam}`;
        ...
        if (pedidos.length === 0) {
          ...
          for (const p of pedidos) {
            ...
            let isPlataformaMlc =
              canal.includes("MERCADO") || nomeLoja.includes("MERCADO");
            let isPlataformaShopee =
              canal.includes("SHOPEE") ||
              nomeLoja.includes("SHOPEE") ||
              obs.includes("SHOPEE");

            let ok = false;
            if (plataforma === "TODOS") ok = true;
            else if (plataforma === "MERCADO_LIVRE" && isPlataformaMlc)
              ok = true;
          ...
          if (
            todosOsPedidos.length >= quantidadeDesejada ||
      ...
      const formatados = ordenados
        ...
        .map((pedido: any) => {
          ...
          if (!numeroLojaOriginal) {
            ...
            const match = info.match(
              /(?:Número|Pedido)\s*[:\-]?\s*([A-Za-z0-9\-_]+)/i
            );
            if (match) numeroLojaOriginal = match[1].trim();
          }
          ...
          return {
            ...
            origem,
            loja: {
              id: pedido?.loja?.id ? String(pedido.loja.id) : undefined,
              nome: lojaName || undefined
            },
            itens: (pedido?.itens || []).map((item: any) => ({
              descricao: item.descricao,
              sku: item.codigo,
