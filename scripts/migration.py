...
def run_migration():
    ...
    try:
        ...
        print(f"✅ SQL carregado: {sql_path}")
            ...
            try:
                ...
                if i == 1:
                    ...
                    if test_response.status_code == 404:
                        print("⚠️  RPC 'exec_sql' não disponível no Supabase")
                ...
                if 200 <= response.status_code < 300:
                    print(f"✅ [{i}/{len(commands)}] Sucesso")
                    print(f"   {cmd[:60]}...\n")
                    success_count += 1
                else:
                    error_count += 1
                    print(f"⚠️  [{i}/{len(commands)}] Status {response.status_code}")
                    print(f"   {response.text[:80]}...\n")
                    
            except requests.exceptions.Timeout:
                error_count += 1
                print(f"⏱️  [{i}/{len(commands)}] Timeout")
                print(f"   {cmd[:60]}...\n")
            except Exception as err:
                error_count += 1
                print(f"❌ [{i}/{len(commands)}] Erro: {str(err)[:60]}")
                print(f"   {cmd[:60]}...\n")
        
        print("="*80)
        print(f"📊 RESUMO DA MIGRATION:")
...
def show_manual_instructions(sql_content):
    ...
    print("1️⃣  Abra: https://app.supabase.com")
    print("2️⃣  Projeto: uafsmsiwaxopxznupuqw")
    print("3️⃣  SQL Editor > New Query")
    print("4️⃣  Cole este SQL:\n")
    print("-"*80)
    print(sql_content)
    print("-"*80 + "\n")
    print("5️⃣  Clique em RUN\n")
