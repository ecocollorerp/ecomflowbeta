...
const CreateProductFromImportModal: React.FC<CreateProductFromImportModalProps> = ({ isOpen, onClose, unlinkedSkuData, onConfirm, generalSettings }) => {
    ...
    const handleConfirm = () => {
        if (newItemName.trim() && primarySku) {
            // ✅ Apenas campos que existem na tabela stock_items conforme MIGRATION_FINAL_UPDATED.sql
            const newItem: Omit<StockItem, 'id'> = {
                ...
                // ❌ Removidos: color, product_type (não existem no schema)
            };
            console.log('📝 Criando novo item com dados:', newItem);
            onConfirm(newItem);
        }
    };
